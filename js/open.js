...
$(document).ready(function() {                     
  $( "#navigation_menu li.collapsed" ).click(function() {
    $( this ).toggleClass( "start_collapsed" );
  });
});
...
$(document).ready(function() {
  var url = window.location.pathname, 
	...
	$('#navigation_menu a').each(function(){
...
/* --------- shareCount -------------- */
function shareCount() {
  var numb = $('.post_soc_icon .social-inp .getshare-counter'),
    allCount = 0;
  numb.each(function () {
    allCount = allCount + Number($(this).html());
  });
  $('.count_shared .quantity').html(allCount);
}
setTimeout(function() {
  shareCount();
  setTimeout(function() {
...
if($('.skill-wrap canvas').exists()){
	var skill = $('.skill-wrap canvas');
...
/* foo2*/
if($(".slider_foo_2 a.prev, .slider_foo_2 a.next").exists()) {
  $(document).ready(function() { 
    $('.slider_foo_2 a.prev, .slider_foo_2 a.next').css("top",  Math.floor($('.foo2 .img_block img').height()/2) );
    setTimeout(function() {
      $('.slider_foo_2 a.prev, .slider_foo_2 a.next').css("top",  Math.floor($('.foo2 .img_block img').height()/2));
    }, 1000);
  });
...
if(typeof($.fn.stellar) !== 'undefined') {
  if(!navigator.userAgent.match(/iPad|iPhone|Android/i) && ($('body').width()) >= 939) {
    if($(".paralax_stellar").exists()) {
      $(document).ready(function() {  
        $(function(){
          $.stellar({
...
//Remove Video
if( navigator.userAgent.match(/iPad|iPhone|Android/i) ) {
  jQuery('.bg_video, .header_video').find('video').remove();
}
...
function carousels() {
  if($(".foo1").exists()) {
    $('.foo1').carouFredSel({
      ...
      onCreate: function(){ 
        ...
        var top = $(this).find('.img_block img').height();
        $(this).closest('.post_slider').find('.prev, .next').css('top', top / 2);
      }
    }).touchwipe({
      wipeLeft: function() {
        $('.foo1').trigger('next', 1);
  ...
  if($(".foo2").exists()) {
      ...
      onCreate: function(){ 
        ...
        var top = $(this).find('.img_block img').height();
        $(this).closest('.slider_foo_2').find('.prev, .next').css('top', top / 2);
      }
    }).touchwipe({
      wipeLeft: function() {
        $('.foo2').trigger('next', 1);
  ...
  if($(".foo8").exists()) {
    $('.foo8').carouFredSel({
      ...
      onCreate: function(){ 
        ...
        var top = $(this).find('.img_block img').height();
        $(this).closest('.slider_foo_8').find('.prev, .next').css('top', top / 2);
      }
    }).touchwipe({
      wipeLeft: function() {
        $('.foo8').trigger('next', 1);
